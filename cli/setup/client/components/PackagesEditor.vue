<template>
  <div class="tp-card p-5 sm:p-6">
    ...
    <div
      v-else-if="packages.panels.length === 0 && packages.modules.length === 0"
      class="flex flex-col items-center py-8 text-center"
    >
      <svg
        class="w-10 h-10 text-base-muted mb-3"
        fill="none"
        viewBox="0 0 24 24"
        stroke="currentColor"
        stroke-width="1.5"
    ...
    <template v-else>
      <!-- Panels -->
      <div
        v-if="packages.panels.length > 0"
        class="mb-6"
      >
        <h3
          class="text-xs font-semibold uppercase tracking-wider text-base-soft mb-3"
        >
          Panels
        </h3>
        <div class="space-y-2">
          ...
          >
            <div
              class="w-8 h-8 rounded-lg bg-secondary-light flex items-center justify-center shrink-0"
            >
              <svg
            ...
            <div class="flex-1 min-w-0">
              <div
                class="text-sm font-medium font-mono text-base-content truncate"
              >
                {{ pkg.name }}
              </div>
            ...
            >
              ...
              >
                <div
                  v-if="
                    busy && busyPackage === pkg.name && busyAction === 'update'
                  "
                  class="tp-spinner-sm"
...
async function uninstallPackage(name) {
  ...
  if (
    !confirm(
      `Uninstall ${name}? This will remove the package and its configuration.`
    )
  )
    return
