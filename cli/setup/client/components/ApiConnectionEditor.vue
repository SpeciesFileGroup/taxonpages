<template>
  <div>
    ...
    <div class="tp-card p-5 sm:p-6">
      ...
      <template v-if="mode === 'browse'">
        ...
        >
          <div class="flex items-start gap-3">
            ...
            <div>
              <p class="font-medium text-sm text-danger">
                Failed to load projects
              </p>
              <p class="mt-1 text-sm text-base-soft">{{ error }}</p>
              <button
                class="tp-btn tp-btn-outline tp-btn-sm mt-3"
                @click="refresh"
              >
                Retry
              </button>
      ...
      >
        <p
          class="text-xs font-semibold uppercase tracking-wider text-base-soft mb-2"
        >
...
const { getConfigValue, setConfigValue, saveConfig, hasUnsavedChanges } =
  useConfig()
